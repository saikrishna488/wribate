  ...
  const fetcharticles = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/user/articles/${category}/?type=${encodeURIComponent(type || '')}&last_id=${encodeURIComponent(lastCreatedAt)}`
      );
        
      const data = res.data;
      if (data.res) {
        
        // Check if we received new articles
        if (data.articles.length > 0) {
          // Properly merge existing and new articles
          setArticles(prevarticles => [...prevarticles, ...data.articles]);
          
          // Set the lastCreatedAt from the last item
          const newLastCreatedAt = data.articles[data.articles.length - 1].last_id;
        ...
        {loading && articles.length === 0 ? (
          <div className="flex justify-center items-center h-64">
            <div className="flex flex-col items-center">
              ...
              <p className="text-blue-600 font-medium">Loading articles...</p>
            </div>
          </div>
        ) : articles.length > 0 ? (
        ...
        {loading && articles.length > 0 && (
          <div className="mt-8 flex justify-center items-center py-8">
            <div className="flex flex-col items-center">
              ...
              <p className="text-blue-600 text-sm font-medium">Loading more articles...</p>
            </div>
          </div>
        )}
